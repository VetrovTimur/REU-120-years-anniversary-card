...
(function() {
    ...
    document.addEventListener('keydown', function(e) {
        // Блокировка F12 и других клавиш
        ...
        for (let combo of BLOCKED_COMBOS) {
    ...
    function antiDebug() {
        ...
        console.log = function() {
            if (arguments.length > 0 && typeof arguments[0] === 'string') {
                ...
                if (msg.includes('debug') || msg.includes('devtools')) {
                    clearPage();
                }
            }
        ...
        // Отключаем возможность остановки на debugger
        Function.prototype.__defineGetter__('caller', function() {
    ...
    function blockAccess() {
        ...
        document.body.innerHTML = `
            <div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#f5f7fa;font-family:sans-serif;text-align:center;padding:20px;">
                <div style="background:white;padding:40px;border-radius:16px;max-width:500px;box-shadow:0 10px 40px rgba(0,0,0,0.1);">
                    <h2 style="color:#dc3545;margin-bottom:12px;">⛔ Доступ запрещён</h2>
                    <p style="color:#5e6b7a;line-height:1.6;">Обнаружена попытка отладки страницы. Доступ к коду ограничен.</p>
                    <button onclick="location.reload()" style="margin-top:20px;padding:12px 32px;background:#052d5a;color:white;border:none;border-radius:10px;font-weight:600;cursor:pointer;">Обновить страницу</button>
                </div>
            </div>
        `;
    ...
    function clearPage() {
        document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#f5f7fa;font-family:sans-serif;">⛔ Доступ запрещён</div>';
    }

    // ===== 5. ОБНАРУЖЕНИЕ ИНСТРУМЕНТОВ РАЗРАБОТЧИКА =====
    ...
    // ===== 6. ЗАЩИТА ОТ КОНСОЛЬНЫХ КОМАНД =====
    // Переопределяем опасные методы
        ...
        if (window[method]) {
            ...
            window[method] = function() {
                ...
                if (str && typeof str === 'string' && (
                    str.includes('localStorage') ||
                    str.includes('document.cookie') ||
                    str.includes('XMLHttpRequest') ||
                    str.includes('fetch')
                )) {
                    console.warn('⛔ Доступ к опасным методам ограничен');
                    return null;
                }
    ...
    // ===== 7. ЗАЩИТА ОТ СОХРАНЕНИЯ СТРАНИЦЫ =====
    document.addEventListener('keydown', function(e) {
        // Блокировка Ctrl+S
    ...
    localStorage.getItem = function(key) {
        ...
        if (key === 'applications' && value) {
            return decryptData(value);
        }
        return value;
    };

    // ===== 9. ПОДМЕНА КОНСОЛЬНЫХ МЕТОДОВ =====
    console.log = function() {};
    ...
    // Оставляем только одну функцию для отладки (но её тоже можно отключить)
    // console._log = console.log;

    // ===== 10. ЗАЩИТА ОТ INSPECT ELEMENT =====
    document.addEventListener('DOMContentLoaded', function() {
        // Удаляем возможность использования инспектора
...
// ============================================
//   ОСНОВНАЯ ЛОГИКА ПРИЛОЖЕНИЯ (ОБФУСЦИРОВАННАЯ)
// ============================================

// ВАЖНО: Этот код обфусцирован для защиты
// В реальном проекте используйте инструменты обфускации
...
(function() {
    ...
    // ============================================
    //   ОСНОВНАЯ ЛОГИКА (ЗАЩИЩЁННАЯ)
    // ============================================

    // Защита от глобального доступа
    function secureFunction(fn) {
        return function(...args) {
            try {
                return fn.apply(this, args);
            } catch(e) {
                console.warn('⛔ Ошибка выполнения');
                return null;
            }
        };
    }
    ...
    // ============================================
    //   ЗАПУСК ПРИЛОЖЕНИЯ (ЗАЩИЩЁННЫЙ)
    // ============================================
    
    document.addEventListener('DOMContentLoaded', function() {
        // Проверка на DevTools при загрузке
        if (detectDevTools && detectDevTools()) {
            return;
        }
