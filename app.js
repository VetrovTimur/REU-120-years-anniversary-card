...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.form-field__input').forEach(input => {
        // При вводе — убираем ошибку, но не показываем успех сразу
        ...
        // При вводе — живая проверка для обязательных полей (показываем успех)
        input.addEventListener('input', function() {
            if (this.dataset.required === 'true' && this.value.trim() !== '') {
                const errorSpan = this.parentElement.querySelector('.field-error');
                if (errorSpan && errorSpan.textContent === '') {
                    // Если ошибки нет — показываем валидность
    ...
    function exportToExcel(triggerDownload = true) {
        ...
        if (triggerDownload) {
            ...
            // Возвращаем blob для автоматической загрузки
